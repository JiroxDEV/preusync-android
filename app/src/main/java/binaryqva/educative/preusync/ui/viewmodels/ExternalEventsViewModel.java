 ...
 * ============================================================================
 * Proyecto: PreuSync
 * Clase: ExternalEventsViewModel.java
 * Versión: v6.0.0
 * Descripción: ViewModel encargado de gestionar los eventos externos. 
...
public class ExternalEventsViewModel extends BaseViewModel<Event> {
    ...
    public ExternalEventsViewModel(@NonNull Application application) {
        super(application);
    ...
    private void loadPage(int page, boolean isRefresh) {
        if (!AppUtils.isConnected(getApplication())) {
            if (isRefresh) {
                repository.getLocalEvents("external", localData -> {
        ...
        String schoolId = PreferenceManager.getInstance(getApplication()).getSchoolId();
        repository.getExternalEvents(start, PAGE_SIZE, schoolId, new Callback<ApiResponse<List<Event>>>() {
            @Override
